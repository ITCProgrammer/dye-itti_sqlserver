...
if (!function_exists('qcf_debug_sqlsrv_connection')) {
    function qcf_debug_sqlsrv_connection() {
        ...
        global $con_db_dying_sqlsrv;
        if (isset($con_db_dying_sqlsrv)) {
            return $con_db_dying_sqlsrv;
        }
        return null;
    }
}
...
if (!function_exists('qcf_debug_session_first')) {
    function qcf_debug_session_first($keys, $default = null) {
        if (!is_array($keys)) {
            $keys = array($keys);
        }
...
if (!function_exists('qcf_debug_upgrade_table_schema')) {
    function qcf_debug_upgrade_table_schema($conn) {
        ...
        $upgradeSql = "
IF COL_LENGTH('db_dying.dbo.tbl_debug_monitor', 'error_trace') IS NULL
BEGIN
    ALTER TABLE db_dying.dbo.tbl_debug_monitor ADD error_trace NVARCHAR(MAX) NULL;
END
";
...
if (!function_exists('qcf_debug_table_ready')) {
    function qcf_debug_table_ready() {
        ...
        $checkSql = "SELECT 1 FROM db_dying.sys.tables WHERE name = 'tbl_debug_monitor'";
        ...
        $createSql = "
IF OBJECT_ID('db_dying.dbo.tbl_debug_monitor', 'U') IS NULL
BEGIN
    CREATE TABLE db_dying.dbo.tbl_debug_monitor (
    ...
    CREATE INDEX IX_tbl_debug_monitor_created_at ON db_dying.dbo.tbl_debug_monitor(created_at DESC);
    CREATE INDEX IX_tbl_debug_monitor_log_type ON db_dying.dbo.tbl_debug_monitor(log_type);
END
";
...
if (!function_exists('qcf_debug_store_db')) {
    function qcf_debug_store_db($record) {
        ...
        $conn = qcf_debug_sqlsrv_connection();
        $sql = "INSERT INTO db_dying.dbo.tbl_debug_monitor
...
if (!function_exists('qcf_debug_build_record')) {
    function qcf_debug_build_record($fatalError = null) {
        ...
        $userName = qcf_debug_safe_text(qcf_debug_session_first(array('usrid', 'user_id10')), 100);
        $sessionUserRaw = qcf_debug_session_first(array('user_id', 'id10'));
        $sessionUserId = is_numeric($sessionUserRaw) ? (int)$sessionUserRaw : null;
