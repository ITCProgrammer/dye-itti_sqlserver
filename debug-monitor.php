...
function qcf_dm_has_column($conn, $columnName) {
    ...
    $sql = "SELECT COL_LENGTH('db_dying.dbo.tbl_debug_monitor', '{$columnName}') AS col_len";
...
function qcf_dm_session_value($keys, $default = null) {
    if (!is_array($keys)) {
        $keys = array($keys);
    }
...
function qcf_dm_get_debug_conn() {
    if (function_exists('qcf_get_db_dying_conn')) {
        return qcf_get_db_dying_conn();
    }
    return null;
}

function qcf_dm_access_state($config) {
    $sessionUser = qcf_dm_session_value(array('usrid', 'user_id10'));
    if ($sessionUser === null) {
        return 'need_login';
    }

    $sessionPass = qcf_dm_session_value(array('pasid', 'pass_id10'));
    if ($sessionPass === null) {
        return 'need_unlock';
    }
    ...
    $currentRole = (string)qcf_dm_session_value(array('lvl_id', 'lvl_id10'), '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dm_action']) && $_POST['dm_action'] === 'truncate') {
    ...
    $dbConnNow = qcf_dm_get_debug_conn();
    $affectedRows = 0;
    $messageParts = array();

    if ($tableReadyNow && (is_resource($dbConnNow) || is_object($dbConnNow))) {
        if ($mode === 'all') {
            $sqlDelete = "DELETE FROM db_dying.dbo.tbl_debug_monitor";
            $stmtDelete = @sqlsrv_query($dbConnNow, $sqlDelete);
        } else {
            $sqlDelete = "DELETE FROM db_dying.dbo.tbl_debug_monitor WHERE created_at < DATEADD(DAY, -?, SYSDATETIME())";
            $stmtDelete = @sqlsrv_query($dbConnNow, $sqlDelete, array($days));
        }

        if ($stmtDelete !== false) {
            $stmtCount = @sqlsrv_query($dbConnNow, "SELECT @@ROWCOUNT AS affected");
            if ($stmtCount !== false) {
...
$dbConn = qcf_dm_get_debug_conn();

if ($tableReady && (is_resource($dbConn) || is_object($dbConn))) {
    ...
    $hasErrorTrace = qcf_dm_has_column($dbConn, 'error_trace');
    ...
    $sql = "SELECT TOP {$limit}
                ...
                {$errorTraceSelect},
                payload_json
            FROM db_dying.dbo.tbl_debug_monitor
            WHERE {$whereSql}
            ORDER BY id DESC";
    ...
    $summarySql = "SELECT
                   ...
                   FROM db_dying.dbo.tbl_debug_monitor
                   WHERE {$summaryWhere}";
    $summaryStmt = @sqlsrv_query($dbConn, $summarySql);
...
<head>
    <meta charset="utf-8">
    <title>DYE-ITTI Debug Monitor</title>
...
<div class="wrap">
    <div class="panel-card">
        ...
        <div class="row">
            <div class="col-sm-7">
                <h3 style="margin-top:0;margin-bottom:8px;">DYE-ITTI Debug Monitor</h3>
                <div class="toolbar">
